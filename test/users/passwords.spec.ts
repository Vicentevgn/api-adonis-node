...
test.group("Password", (group) => {
  ...
  test("it should create and reset password token", async (assert) => {
    const user = await UserFactory.create();
  ...
  test("it should return 422 when required data is not provided or date is invalid", async (assert) => {
  ...
  test("it cannot reset password when token is expired", async (assert) => {
    const user = await UserFactory.create();
